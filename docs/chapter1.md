...
Another way to characterize nonnegative, positive, irreducible and primitive matrices, is to analyze their image on vectors with nonnegative entries. Consider 

\[\RR^n_+=\{x\in \RR^n\mid x_1,\ldots,x_n\geq 0\}\quad \text{and}\quad \RR^n_{++}=\{x\in \RR^n\mid x_1,\ldots,x_n>0\}.\] 

Note for instance that a matrix $A\in\RR^{n\times n}$ is nonnegative if, and only if, $Ax\in\RR^n_+$ for all $x\in\RR^n_+$.
<section markdown="block" class="theorem">
**Proposition.** Let $A\in\RR^{n\times n}$ be a nonnegative matrix.

- $A$ is positive if, and only if, $Ax\in\RR^n_{++}$ for all $x\in\RR^n_+\setminus\{0\}$.
- $A$ is irreducible if, and only if, there exists an integer $m$ such that $\sum_{k= 0}^m A^{k}x\in\RR^n_{++}$ for all $x\in\RR^n_+\setminus\{0\}$ (see Theorem ...).
- $A$ is primitive if, and only if, there exists an integer $m$ such that $A^{m}x\in\RR^n_{++}$ for all $x\in\RR^n_+\setminus\{0\}$ (see Theorem ...).
</section>
...
We can now state the celebrated theorem of Perron and Frobenius which provides conditions for the existence of a nonnegative eigenvector, i.e. an eigenvector in $\RR^n_+$, the existence and uniqueness of a positive eigenvector, i.e. an eigenvector in $\RR_{++}^n$, and the convergence of a sequence towards a unique positive eigenvector. Note that, by unique eigenvector, we mean unique up to scale so that $u$ and $\alpha u$ are considered to be the same eigenvector.

<section markdown="block" class="theorem">
**Theorem (Perron-Frobenius).** Let $A\in\RR^{n\times n}$ be a matrix and $\|\cdot\|$ a norm on $\RR^n$.

- If $A$ is nonnegative, then there exists an eigenvector $u\in\RR^n_+$ such that $Au=\rho(A)u$.
- If $A$ is irreducible, then $u$ is the unique up to scale eigenvector of $A$ in $\RR^n_+$. Furthermore $u\in\RR^n_{++}$ and $\rho(A)$ is a geometrically simple eigenvalue.
- If $A$ is primitive, then $\rho(A)$ is algebraically simple and for every $x^0\in \RR^n_+\setminus\{0\}$, the sequence $(x^k)_{k=0}^{\infty}\subset\RR^n$ defined as $x^k = A x^{k-1}$ for all $k\geq 1$ satisfies
